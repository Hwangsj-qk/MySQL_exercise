...
2. Doctors 테이블: 의사 정보를 저장합니다.
    ...
    - `Specialization` 열은 비어 있지 않아야 하며(`NOT NULL`), 
       의사의 이메일(`Email`)은 고유해야 합니다(`UNIQUE`).
    */
...
3. Appointments 테이블: 예약 정보를 저장합니다.
    ...
    - `Appointments` 테이블을 생성하세요. `PatientID`와 `DoctorID`에 외래 키 제약 조건을 추가하고, 
	   `AppointmentDate`는 현재 날짜 이후로 설정할 수 없지만, 예약 날짜는 2020년 이후여야 한다는 
       `CHECK` 제약조건을 추가합니다.
*/
